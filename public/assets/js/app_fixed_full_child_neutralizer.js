...
/* === PERFIL (HIJO): neutralizador robusto de precio anclado/readonly ===
   - Mantiene #modalChildPrecio editable y con 0.00 (o el valor que definas)
   - Elimina clones (modalChildPrecio_display), anclas ([data-price-mount]/[data-price-slot])
   - Evita duplicados de input[name="soles"]
   - Reaplica tras mutaciones del DOM (MutationObserver)
*/
(function(){
  var pm = document.getElementById('perfilModal');
  if (!pm) return;

  function ensureEditableZero(){
    var f = pm.querySelector('form'); if (!f) return;
    // Real input (preferimos #modalChildPrecio si existe)
    var real = f.querySelector('#modalChildPrecio') || f.querySelector('input[name="soles"]');
    if (real) {
      real.readOnly = false;
      real.removeAttribute('readonly');
      real.classList.remove('bg-light');
      // deja 0.00; si lo quieres vacío, cambia a ''
      if (!real.value || real.value === '0' || real.value === '0.0' || real.value === '0.00') {
        real.value = '0.00';
      }
    }
    // eliminar clones y anclas
    f.querySelectorAll('#modalChildPrecio_display, [data-price-mount], [data-price-slot]').forEach(function(n){ n.remove(); });
    // eliminar inputs 'soles' extra
    var list = f.querySelectorAll('input[name="soles"]');
    list.forEach(function(el){
      if (real && el !== real) el.remove();
    });
  }

  function isChild(){ return !!(pm.dataset && pm.dataset.context === 'child'); }

  // Antes de que otros scripts muten (captura)
  pm.addEventListener('show.bs.modal', function(ev){
    if (!isChild()) return;
    ensureEditableZero();
  }, true);

  // Ya visible: forzar y reintentar por si otro handler tocó después
  pm.addEventListener('shown.bs.modal', function(ev){
    if (!isChild()) return;
    ensureEditableZero();
    setTimeout(ensureEditableZero, 0);
    setTimeout(ensureEditableZero, 30);
    requestAnimationFrame(ensureEditableZero);
  }, true);

  // Observa mutaciones mientras el modal esté en modo hijo
  var mo = new MutationObserver(function(records){
    ...
    for (var i=0;i<records.length;i++){
      ...
      if (r.type === 'attributes') { need = true; break; }
    }
    if (need) ensureEditableZero();
  });
  mo.observe(pm, { childList: true, subtree: true, attributes: true });

  // Al cerrar, limpiar contexto para no contaminar otros flujos
