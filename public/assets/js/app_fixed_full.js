...
setVal(perfilModal, 'input[name="soles"]', '0.00'); // hijo: editable (default 0.00)
try {
...
// === Guard: Perfil (hijo) fuerza precio editable y 0.00 ===
(function(){
  try{
    var pm = document.getElementById('perfilModal');
    if(!pm) return;
    pm.addEventListener('show.bs.modal', function(ev){
      if (pm.dataset && pm.dataset.prefill === '1') {
        var inp = pm.querySelector('input[name="soles"]');
        if (inp) { inp.value = '0.00'; inp.readOnly = false; inp.removeAttribute('readonly'); }
      }
    }, true);
  }catch(e){ console.warn('[guard child price]', e); }
})();
